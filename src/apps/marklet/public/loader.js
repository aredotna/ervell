;(function() {
  ...
  function createStyle() {
    ...
    markletCSS =
      '' +
      '#arena_frame,#arena_div{width:' +
      markletWidth +
      'px !important;height:' +
      markletHeightContracted +
      'px !important;position:fixed;top:20px;right:20px;border:5px solid #e7e7e5;border-radius:0.25em;overflow-x:hidden;overflow-y:auto;max-height:calc(100% - 50px);}' +
      '#arena_frame.is-expanded{height:' +
      markletHeightExpanded +
      'px !important}' +
  ...
  function closeBookmarklet() {
    ...
    if (markletFrame && markletFrame.parentNode)
      markletFrame.parentNode.removeChild(markletFrame)
    if (markletDiv && markletDiv.parentNode)
      markletDiv.parentNode.removeChild(markletDiv)
    if (markletStyle && markletStyle.parentNode)
      markletStyle.parentNode.removeChild(markletStyle)
  }
  ...
  function drop(e) {
    ...
    for (var i in e.dataTransfer.types) {
      data[e.dataTransfer.types[i]] = e.dataTransfer.getData(
