...
import globalNavElementsQuery from 'react/components/GlobalNavElements/queries/globalNavElements';
...
class GlobalNavElements extends PureComponent {
  ...
  signOut = (err) => {
    ...
    const { extensions: { code } } = err.graphQLErrors[0];
  ...
  render() {
    ...
    return (
      <Query query={globalNavElementsQuery} ssr={false} skip={isLoggedOut} onError={this.signOut}>
        {({ data }) => {
          const components = [];

          // Merge client-side fetched me data along with the serialized data
          const me = isLoggedIn ? { ...serializedMe, ...((data && data.me) || {}) } : null;
