...
export default class ChannelPreviewBlocks extends PureComponent {
  static propTypes = {
    blocks: PropTypes.arrayOf(propType(konnectableChannelPreviewBlockFragment))
      .isRequired,
  ...
  componentDidMount() {
    ...
    const imageUrls = blocks
      .map(({ preview_image_url }) => preview_image_url)
      .filter(Boolean)

    preloadImages(imageUrls)
      .then(() => this.setState({ mode: 'ready' }, () => this.play()))
      .catch(() => this.setState({ mode: 'ready' }, () => this.play()))
  }
  ...
  render() {
    ...
    const Inner = {
      ...
      Image: () => <Img src={block.preview_image_url} alt="Preview" />,
      Link: () => <Img src={block.preview_image_url} alt="Preview" />,
      Embed: () => <Img src={block.preview_image_url} alt="Preview" />,
      Attachment: () => (
        <React.Fragment>
