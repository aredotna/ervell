...
import channelContextQuery from 'v2/components/ModalFullBlockWithChannelContext/queries/channelContext'
...
export default class ModalFullBlockWithChannelContext extends PureComponent {
  static propTypes = {
      ...
      <Modal onClose={this.close} Dialog={ModalFullscreenDialog}>
        <Query query={channelContextQuery} variables={{ id: channel_id }}>
          {({ data, loading, error }) => {
            ...
            return <ModalFullBlock id={id} ids={ids} onClose={this.close} />
          }}
        </Query>
      </Modal>
    )
