      ...
      <RadioOptions value="basic" onSelect={action('onSelect')}>
        ...
        <RadioOptions.Option value="lifetime" disabled>
          {({ selected }) => (
            <Text f={4} mb={3} selected={selected}>
              <strong>
                Lifetime Premium (disabled) {JSON.stringify({ selected })}
              </strong>
            </Text>
          )}
        </RadioOptions.Option>
  ...
  .add('sizing', () => (
    <Specimen>
      <RadioOptions value="basic" onSelect={action('onSelect')} size="1em">
        <RadioOptions.Option value="one">{() => 'One'}</RadioOptions.Option>

        <RadioOptions.Option value="two">{() => 'Two'}</RadioOptions.Option>
